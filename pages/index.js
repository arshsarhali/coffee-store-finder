...
export default function Home(props) {
  ...
  const {handleUserLocation, latLong, locationErrorMsg, loadingLocation} = userLocation();

  console.log({latLong,locationErrorMsg})

  const [coffeeStoresNear, setCoffeeStoreNear] = useState('');
...
if(latLong){
  try{
    ...
    setCoffeeStoreNear(fetchedCoffeeStores)
  } catch(err){
    console.log(err);
...
setCoffeeStoresByLocation();
  ...
  return (
    <div className={styles.container}>
     ...
     {coffeeStoresNear.length>0 &&( <div className={styles.sectionWrapper}>
<h2 className={styles.heading2}>Stores near me</h2>

<div className={styles.cardLayout}>
{coffeeStoresNear.map(coffeeStore=>(
